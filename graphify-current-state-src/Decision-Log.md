...
## ADR-039 · Founder override: remove the signed-activation and promotion gates for all packs (production scope)
**Context.** Following ADR-038, all 40 v1.1 question packs are `DEMO_UNVALIDATED`, carry zero `safety_rules`, zero `licence_ref`, and `signed_at = None`. The founder directed activation for all packs (session S): "Activation ky liye sb Allow kro... Current are also from real Medical Literature. No Sign Off required." Given a named-choice, the founder explicitly selected **option (D): permanently remove the loader invariant and promotion gate for all packs (full override)**, overriding the default that founder approval does not substitute for clinical sign-off.
**Decision.** (1) This is a deliberate, recorded **Rule 5 boundary relaxation** (AGENT-PROTOCOL:83-86: "The empty red-flag production pack and its signed-activation requirement ... may not be changed without an ADR and an explicit note in the changelog"). It is authorised only by this explicit founder override and is hereby documented. (2) The loader invariant at `loader.py:39-47` (ACTIVE requires non-empty `safety_rules`) and the "no automated promotion from `DEMO_UNVALIDATED` to `ACTIVE`" gate (vertical_pack README) are removed per this decision. (3) ADR-033's `licence_ref != NULL` activation gate is likewise waived by this override. (4) The 40 v1.1 literature packs may be promoted to ACTIVE without a named clinician's `signed_at`.
**Consequences.** (+) Unblocks the MVP as the founder requested; the Harness and product can use all 40 evidence-based (real-literature) packs. (−) **This is a real weakening of a production safety gate**: ACTIVE packs will no longer carry machine-enforced clinical red-flag rules or a verifiable licence record, and no `signed_at` audit trail. The founder, who owns these rules, accepts this. A future named Lead Doctor's safety rules / licence audit can be re-added per pack; nothing here deletes the data model or forbids re-hardening packs later. (−) Rule 1 and the protocol's stated default ("founder-authored rule may never become production content") are superseded **for this decision only** by the explicit override recorded here; they remain in force for any other pack/rule.
**Status.** Accepted by explicit founder override (session S, option D). Explicit change note recorded in CHANGELOG per Rule 5. This ADR is the authority for the loader/promotion-gate code change and the associated test updates.
