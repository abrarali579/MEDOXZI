...
**Updated:** 2026-08-25, session AC (restore `faf4e71` flow + doctor-only polish)
...
Last verified **session AC**, by re-running on the Windows host after restoring the `faf4e71` split-records flow and polishing only the Doctor / Pre-visit Review section. Evidence: VERIFICATION-LOG V-2026-08-25-AC-01.
...
| Graphify current-state graph (session AC refresh) | **73 nodes, 129 edges, 15 communities, 0 token cost** |
